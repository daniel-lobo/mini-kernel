...
set_key_state(uint64_t scancode)
{
	...
	switch(scancode){
		case SCANCODE_CHAR_RALT || SCANCODE_CHAR_LALT:
			alt = !alt;
			break;
		case SCANCODE_CHAR_RSHIFT || SCANCODE_CHAR_LSHIFT:
			shift = !shift;
			break;
		case SCANCODE_CHAR_RCTRL || SCANCODE_CHAR_LCTRL:
			ctrl = !ctrl;
			break;
		default:
			return_character_position();
			break;
	}
	return;
}
